...
// Function for populating the mWorldChain and mLengths variable.
// The mLengths variable stores the length between mIKChain[currentIdx] and mIKChain[currentIdx+1].
// The mWorldChain stores the world positions of the joints in the mIKChain variable.
...
// Function to update the joint rotations in the mIKChain.
void FABRIKSolver::WorldToIKChain() {
	unsigned int size = Size();
	if (size == 0) { return; }

    // loop through all joints in mIKChain
...
bool FABRIKSolver::Solve(const Transform& target) {
    ...
    // loop until the number of steps defined is exceeded
	for (unsigned int i = 0; i < mNumSteps; ++i) {
		vec3 effector = mWorldChain[last];

        // if the length between the goal and effector is close enough
