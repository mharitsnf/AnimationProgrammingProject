#include "ChapterEightApp.h"
#include <iostream>
#include <cstdarg>

void ChapterEightApp::Initialize() {
...
void ChapterEightApp::Render(float inAspectRatio) {
	mat4 model;
...
VectorFrame ChapterEightApp::MakeFrame(float time, const vec3& value) {
	return MakeFrame(time, vec3(), value, vec3());
}

QuaternionFrame ChapterEightApp::MakeFrame(float time, const quat& value) {
	return MakeFrame(time, quat(0, 0, 0, 0), value, quat(0, 0, 0, 0));
}

ScalarFrame ChapterEightApp::MakeFrame(float time, float in, float value, float out) {
	ScalarFrame result;
	result.mTime = time;
	result.mIn[0] = in;
...
VectorFrame ChapterEightApp::MakeFrame(float time, const vec3& in, const vec3& value, const vec3& out) {
	VectorFrame result;
	result.mTime = time;
	result.mIn[0] = in.x;
	result.mIn[1] = in.y;
...
QuaternionFrame ChapterEightApp::MakeFrame(float time, const quat& in, const quat& out, const quat& value) {
...
ScalarTrack ChapterEightApp::MakeScalarTrack(Interpolation interp, int numFrames, ...) {
	ScalarTrack result;
...
VectorTrack ChapterEightApp::MakeVectorTrack(Interpolation interp, int numFrames, ...) {
	VectorTrack result;
	result.SetInterpolation(interp);
	result.Resize(numFrames);
...
QuaternionTrack ChapterEightApp::MakeQuaternionTrack(Interpolation interp, int numFrames, ...) {
